...
框，页面底下列出要你拍板的问题。大活涉及多个模块时，开工前还会画一张「我打算怎么搭」图（模块怎么分、
数据怎么流），让你知道要怎么做、不对随时插话，不用你批；分层架构那种靠格子对
...
| 探方案 | 不探，直接做 | 技术取舍 AI 定、codex 审；影响你用起来的翻译成后果再问你；界面类出原型让你挑；多模块的画「我打算怎么搭」图；定稿后三行白话告知方向就开工 |
...
不讲语言、库、框架这层的名词；模块、接口、注册表、分层这类架构词直接用，第一次带一句解释。方案定了用三行
白话告诉你（做什么 / 你会看到什么 / 不做什么），多模块的附上「我打算怎么搭」图，然后直接开工，方向不对你随时叫停；
...
0.13.2（2026-09-24）把「我打算怎么搭」图改回大活默认画，架构词也放开了。0.11 起按"用户不懂技术、
不审方案"把这张图收成了按需才画，后来用户澄清："我不懂其他技术，比如html之类。基础代码框架，概念
我是懂的，比如流程图"——不懂的是代码层，架构层是懂的，一起藏掉就改过头了。现在架构层照旧由 AI 定，
但画出来给他看；代码层照旧不问。这是试行，两三个大活后拿实际例子问他一次：继续先看后反馈，还是有哪些
架构取舍想提前自己选，按他的回答再改决策边界。0.13.3 按 codex 补审收紧了三处：一条明显路的大活也画图；
这张图上内部架构的疑点不问用户、不走"答完才算完"的循环；clarify-needs 里"架构名一律翻译掉"收窄到需求阶段。
