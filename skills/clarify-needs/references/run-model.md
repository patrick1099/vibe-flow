...
| **第二层：我打算怎么搭** | 模块怎么分、数据从哪来到哪去、存在哪 | `vibe-flow` §4 探方案，**按需**：用户说想看，或向决策者解释方案时自己用得上 | `build` |

第一层**不许出现实现**（没有函数名、库名、模块名）——那是需求，不是方案。第二层才讲怎么搭，**默认不画**：技术方案由 AI 和决策者负责，用户不审；画了也只是让用户知道我会怎么做，纯技术内部选择照旧不许反问用户，`guess` 只标会改变用户体验或数据的点。
...
- **第二层**：不默认画，只在用户说想看、或向决策者解释方案时自己用得上才画。
