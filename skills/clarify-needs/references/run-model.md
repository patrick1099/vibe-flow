...
| **第二层：我打算怎么搭** | 模块怎么分、数据从哪来到哪去、存在哪 | `vibe-flow` §4 探方案，好档且本次动到两个以上职责模块、或数据跨好几个环节时**默认画**（一条明显路也画） | `build` |

第一层**不许出现实现**（没有函数名、库名、模块名）——那是需求，不是方案。第二层才讲怎么搭，停在**架构层**：模块、各管什么、数据怎么流、存在哪，可以用模块、接口、注册表、分层这类架构词（首次带一句解释），不写库名、语言、函数名。用户懂这一层、不懂代码，画出来是让他知道我会怎么做、不对能及时插话——是告知不是审批，不等回复就开工；纯技术内部选择照旧不许反问用户，内部架构拿不准的由 AI 按 `vibe-flow` §7 自己处理，`guess` 只标此前没确认过、会改变用户体验或数据的点。
...
- **画第二层**：好档，且本次动到两个以上职责模块，或数据跨好几个处理 / 存储环节（按这次实际动到的结构算，不数整个仓库）。只有一条明显路也画。单模块的小改不画；用户说想看时照画。
- **第一层不画**：没流程的（一个静态配置页、一张表）→ 改给界面草样，那是 vibe-flow §4 的原型。这条只管第一层，第二层的分层架构图不受它影响。
- **微任务** → 两层一律不画（缩放闸）。
- 用户说"别画了直接做"→ 立刻收。
...
- **我猜的标 `guess`**（黄色虚线）。确认一个就去掉 `guess`。第一层凡是猜的都标；第二层只标此前没确认过、会改变用户体验或数据的点，内部架构的疑点不标、不问用户。
...
3. **第二层不走这个等答案的循环**：没有 `--q` 就出页面、说一句"页面已经打开"然后开工；有 `--q`（只会是影响用户的新问题）就只等这几处的答案，其余照 `vibe-flow` §4 直接做。
