...
| **第二层：我打算怎么搭** | 模块怎么分、数据从哪来到哪去、存在哪 | `vibe-flow` §4 探方案，好档且有两个以上模块或数据要流过好几处时**默认画** | `build` |

第一层**不许出现实现**（没有函数名、库名、模块名）——那是需求，不是方案。第二层才讲怎么搭，停在**架构层**：模块、各管什么、数据怎么流、存在哪，可以用模块、接口、注册表、分层这类架构词（首次带一句解释），不写库名、语言、函数名。用户懂这一层、不懂代码，画出来是让他知道我会怎么做、不对能及时插话——是告知不是审批，不等回复就开工；纯技术内部选择照旧不许反问用户，`guess` 只标会改变用户体验或数据的点。
...
- **画第一层**：要做的东西有**流程**——多步、有分岔、有出错路径、有人要介入的环节。
- **画第二层**：好档，且有两个以上模块或数据要流过好几处。单模块的小改不画；用户说想看时照画。
