...
## 6. 当天追加（v0.2.0）：scan-field 改成两头

用户合并当天就指出 scan-field 的定位别扭："扫同款按理是刚开始的时候，结果这是结尾……或许有点鸡肋，又或者我还需要一个 skill 补设计阶段？"

**病根不是鸡肋，是时点的理由错了。** 旧 scan-field 的"事后"是从 §2 那张表里**与 cut-scope 对称**继承来的（对内剪 / 对外扫，一对），不是从"扫同款的价值在哪个时点最大"推出来的。而这两件事的时间性根本不同：

- **设计膨胀只会事后发生** → cut-scope 放事后是对的，没得选。
- **"不知道同款存在"的代价在开工前最大** → 这也正是用户偏好里写的"开新 skill/插件前先搜市面同款"。

证据是旧版**自己内部就矛盾**：判断准则写着"规则不是禁止重写，是**重写前**先看过同款"，而何时用/不用写着"**不用**：还没做出可对比的东西时"。

**但事后那一次不废**，晚扫能拿到早扫拿不到的三样：搜索锚不同（开工前是你*打算*做的东西，容易搜偏；完工后是 BLUEPRINT 里的真定位）、产物性质不同（决策输入 vs 迭代输入）、该花的成本不同（开工前花多了拖慢开工）。

**所以不新建 skill，改成一个 skill 管两头**——形态照 `clarify-needs`（它当年正是从只管回头的 reality-check 改成开口/回头两头的，已验证好用）。拆两个 skill 会复制 80% 正文（搜索 fan-out、原创度判据、"真正的浪费只有一种"）并多一份 always-on 开销。

落地：
- `scan-field` v0.2.0：一进门先分岔；**开口那头薄**（一屏、几分钟、主线直接搜、不落文档，结论只有"有现成的直接用 / 有邻近款抄点子 / 没搜到放心做"三种，且**从不劝退**——用户知情后仍要自己做就闭嘴）；**回头那头厚**（保留 subagent fan-out + 原创度对比表 + 升级方向文档）。
- **加缩放闸**（否则违反"小事要省"）：开口那头只在"要新建能拿出去的东西"或"打算重写替换已有的东西"时跑；日常改功能 / 加子命令 / 修 bug / 一次性微脚本不跑，不命中一个字都不提。
- `vibe-flow` §3 从"轻扫市面同款……真要出完整对比文档才用 scan-field"（合并当天为绕开这个矛盾写的权宜之计，把一条明确的用户偏好压成一行提示）改成**分两问、从近到远**：① 自己有没有（查 script-manager 的 INDEX）② 市面有没有（交给 scan-field 开口那头，缩放闸归它自己判）。§9 那行也标明是"回头那头"。
